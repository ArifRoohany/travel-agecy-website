...
function DestinationsGrid() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.utils.toArray(".dest-card").forEach((card, index) => {
        gsap.fromTo(
          card,
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-6">
        <div className="dest-grid-heading flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-14">
          <div>
            ...
            <p className="mt-4 text-slate-600 dark:text-slate-400 max-w-2xl leading-7">
              Explore our complete destination collection with trusted planning,
              affordable packages and professional travel support.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {destinations.map((item) => (
            <div
              key={item.name}
              className="dest-card group rounded-[34px] bg-white dark:bg-slate-900 shadow-xl hover:shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden hover:-translate-y-2 transition duration-500"
            >
